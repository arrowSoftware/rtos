...
LOCAL BOOL fdIsWP(int drive);
LOCAL void fdRelease(void);
LOCAL void fdSelect(int fdType, int drive);
LOCAL STATUS fdRecalibrate(int drive);
...
LOCAL STATUS fdBlkRd(FD_DEV *pFdDev, int startBlk, int nBlks, char *buffer);
LOCAL STATUS fdBlkWrt(FD_DEV *pFdDev, int startBlk, int nBlks, char *buffer);
LOCAL STATUS fdIoctl(FD_DEV *pFdDev, int cmd, ARG arg);

LOCAL void fdIntr(int ctrl);
LOCAL void fdOff(void);
LOCAL STATUS fdBlkRW(FD_DEV *pFdDev, int startBlk, int nBlks,
		     char *buffer, int direction);
LOCAL STATUS fdRW(int fdType, int drive, int cylinder, int head, int sector,
		  void *pBuf, int nSecs, int direction);
LOCAL STATUS fdFormat(int fdType, int drive, int cylinder, int head,
		      int interleave);

LOCAL int fdCmdSend(unsigned char *pCmd, int nBytes);
LOCAL int fdResult(unsigned char *buffer, BOOL immediate, int nBytes);
LOCAL STATUS fdIntSence(BOOL seekEnd);
...
STATUS fdDrvInit(int vector, int level)
{
  if (fdDrvInstalled)
    return OK;
  ...
  intConnectFunction(vector, fdIntr, (void *) 0);
...
LOCAL BOOL fdIsWP(int drive)
{
  unsigned char cmd[FD_CMD_SENSEDRIVE];
...
/*******************************************************************************
 * fdSelect - Select and turn of drive
 *
 * RETURNS: N/A
 ******************************************************************************/
...
{
  FD_TYPE *pFdType;
  unsigned char cmd[FD_CMD_LEN_SPECIFY];
...
LOCAL STATUS fdRecalibrate(int drive)
{
  int i;
  STATUS ret[2];
  unsigned char cmd[FD_CMD_LEN_RECALIBRATE];
...
LOCAL STATUS fdStatusChk(FD_DEV *pFdDev)
{
  ...
  wdStart(fdWdId, sysClockRateGet() * fdWdSeconds,
	  (FUNCPTR) fdOff, (ARG) 0);
...
{
  STATUS ret;
  unsigned char cmd[FD_CMD_LEN_SEEK];
		     ...
		     char *buffer, int direction)
{
  BLK_DEV *pBlkDev;
  FD_TYPE *pFdType;
  STATUS status, rwStatus;
  int head, cylinder, sector, nSecs;
  int i, tryRW, tryRecalibrate, trySeek;
  ...
  for (i = 0; i < nBlks; i += nSecs) {
    ...
    while ( (rwStatus = fdRW(pFdDev->fdType, pFdDev->drive, cylinder, head,
			     sector, buffer, nSecs, direction)) != OK ) {
      ...
      } /* End if read/write timeout */

    } /* End while read/write track data */
		  ...
		  void *pBuf, int nSecs, int direction)
{
  FD_TYPE *pFdType;
  unsigned int nBytes;
  unsigned char cmd[FD_CMD_LEN_RW], result[7];
  char fd_mt;
  int ret;
  ...
  /* Else write */
    ...
    cmd[0] = FD_CMD_WRITE |
	     (fd_mt << 7) |
	     (pFdType->mfm << 6);
    memcpy((char *) sysFdBuf, pBuf, nBytes);
  ...
  if (ret == 0) {
    ...
    if ( (result[0] & 0xc0) == 0x00 ) {
      ...
      if (direction == O_RDONLY)
        memcpy(pBuf, (char *) sysFdBuf, nBytes);
		      ...
		      int interleave)
{
  ...
  int i, sector;
  char *pBuf;
  unsigned char cmd[FD_CMD_LEN_FORMAT], result[7];
  int ret;
  ...
  /* Setup dma for writing */
  if ( dmaSetup(O_WRONLY, (void *) sysFdBuf, pFdType->sectorsPerTrack * 4,
		FD_DMA_CHAN) != OK )
    return ERROR;
...
LOCAL int fdCmdSend(unsigned char *pCmd, int nBytes)
{
  ...
  for (i = 0; i < nBytes; i++) {
    ...
    /* Check if FDC is ready for input */
...
{
  int ret;
  unsigned char cmd[FD_CMD_LEN_SENSEINT], result[2];
