...
static u_int16_t border_color	= 0x0000;
static u_int16_t font_color	= 0xffff;

void bfont_border_color(int r, int g, int b)
{
 	border_color = (r << 16) | (g << 8) | (b << 0);
}

void bfont_font_color(int r, int g, int b)
{
 	font_color = (r << 16) | (g << 8) | (b << 0);
}
...
extern u_int8_t* get_font_address();
asm(
"	.text\n"
"	.align 2\n"
"_get_font_address:\n"
...
void bfont_draw_thin(u_int16_t *buffer, int bufwidth, int opaque, int c, int iskana) {
	...
	for (y=0; y<24; ) {
		/* Do the first row */
		word = (((u_int16_t)ch[0]) << 4) | ((ch[1] >> 4) & 0x0f);
		for (x=0; x<12; x++) {
			if (word & (0x0800 >> x))
				*buffer = font_color;
			else {
		...
		/* Do the second row */
		word = ( (((u_int16_t)ch[1]) << 8) & 0xf00 ) | ch[2];
		for (x=0; x<12; x++) {
			if (word & (0x0800 >> x))
				*buffer = font_color;
			else {
...
void bfont_draw_wide(u_int16_t *buffer, int bufwidth, int opaque, int c) {
	u_int8_t	*ch = bfont_find_char_jp(c);
	u_int16_t	word;
	int	x, y;
