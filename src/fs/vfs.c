...
int vgetino (
    mount_t *      pMount,     /* ptr to file system mount */
...
int  vncioCreate (
    ...
    if (S_ISREG (mode)) {
        ...
        error = vnioConnect (pDevHeader, filename, NULL, oflags, mode,
                             CN_CREATE, CN_NOFOLLOW, vnioMkdir);
    } else if (S_ISLNK (mode)) {
        error = vnioConnect (pDevHeader, filename, symlink, oflags, mode,
                             CN_CREATE, CN_NOFOLLOW, vnioSymlink);
    } else {
...
int vncioRead (
    ...
    if ((pFileDesc->fd_mode & 3) == O_WRONLY) {
        errnoSet (EBADF);
        return (ERROR);
    }
...
int vncioWrite (
    ...
    if ((pFileDesc->fd_mode & 3) == O_RDONLY) {
        errnoSet (EBADF);
        return (ERROR);
    }
...
int vncioIoctl (
    ...
    void *       pData         /* additional parameter */
    ) {
    int              error;
    int              rv;
    int              eof;
    ...
    switch (command) {
        ...
        case FIOSEEK:
            ...
            if ((pData == NULL) || (*((off_t *) pData) < 0)) {
                error = EINVAL;
                break;
            }
            pFileDesc->fd_off = *((off_t *) pData);
            break;
        ...
        case FIOSTATGET:
            ...
            pStat->st_blocks  = vattr.va_bytes / vattr.va_blksize;
            return (OK);
        ...
        case FIOREADLINK:
            ...
            mountLock (mp);
            error = vnioReadlink (pFileDesc->fd_vnode, (struct iovec *) pData,
                                  &bytesRead);
            if (error != OK) {
                errnoSet (error);
                bytesRead = ERROR;
        ...
        case FIOREADDIR:
            ...
            readdirArgs.dep = (struct dirent *) pData;
            ...
            return ((int) (readdirArgs.eof ? NULL : pData));
