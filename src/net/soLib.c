...
int socreate(int domain, struct socket **aso, int type, int proto)
{
  ...
  if (_func_selWakeupListInit != NULL) {
    ...
    ( *_func_selWakeupListInit) (&so->so_selWakeupList);
    so->selectFlag = TRUE;
...
void sofree(struct socket *so)
{
  ...
  if (so->so_state & SS_NOFDREF) {
    ...
    if (so->selectFlag) {
      ...
      ( *_func_selWakeupListTerminate) (&so->so_selWakeupList);
      so->selectFlag = FALSE;
...
void sbwakeup(struct socket *so, struct sockbuf *sb, SELECT_TYPE selType)
{
  ...
  if (sb->sb_sel) {
    ...
    if ( (so->selectFlag) && (_func_selWakeupAll != NULL) )
      ( *_func_selWakeupAll) (&so->so_selWakeupList, selType);
