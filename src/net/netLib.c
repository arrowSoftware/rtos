...
STATUS netLibInit(void)
{
  ...
  netWorkRing = ringCreate(NET_LIB_RING_SIZE);
  if (netWorkRing == NULL)
    return ERROR;
...
void netTask(void)
{
  ...
  while (1) {
    ...
    /* Process work list */
    while ( ringIsEmpty(netWorkRing) == FALSE ) {

      /* Get work node, kill task if error */
      if ( ringBufferGet(netWorkRing, (char *) &node,
		 	       ...
		 	       ARG param3, ARG param4)
{
  ...
  ringPut = ringBufferPut(netWorkRing, (char *) &node, sizeof(node));
