...
/* scanf.c - Generic text input routines */
...
int sscanf(const char *sp, const char *fmt, ...)
{
	...
	f._flags = __SRD;
	f._p = (unsigned char *) sp;
	f._bf._base = f._p;
	f._bf._size = BUFSIZ;
	f._r = f._bf._size;

	va_start(ptr, fmt);
	rv = vfscanf(&f, fmt, ptr);
	va_end(ptr);
	return rv;
}
...
static int __strtold(long double *ld, struct scan_cookie *sc);
						   /*01234567890123456 */
static const char spec_s[]  = "%n[csoupxXidfeEgG";
#else
static const char spec_s[]  = "%n[csoupxXid";
#endif
/* radix[i] <-> spec_s[i+5]   o   u   p   x   X  i   d */
