...
/* printf.c - Generic text output routines */
...
#define INCLUDE_LONG_LONG_ERROR   0
#define INCLUDE_FLOAT_ERROR       0
...
int vsnprintf(char *sp, size_t size, const char *fmt, va_list ap)
{
	...
	/* Initialize vritual FILE structure */
	stdioFpInit(&f);
	 ...
	 */
	f._flags = __SWR;			/* Writable string */
	f._p = (unsigned char *) sp;		/* Target string */
	f._bf._base = f._p;			/* Store base address */
	f._bf._size = ((int) size < 0) ? BUFSIZ : size;
	f._w = f._bf._size;
...
int vdprintf(int fd, const char *fmt, va_list ap)
{
	...
	/* If any of the standard file descriptors */
	if (fd >= 0 && fd <= 3)
		f = stdioFp(fd);

	/* Else not a standard file descriptor */
	else {
...
#if defined(INCLUDE_LONG_LONG) || INCLUDE_LONG_LONG_ERROR
...
#if !defined(INCLUDE_FLOAT) && INCLUDE_FLOAT_ERROR
...
#define upcase i
	...
	while (*fmt) {
...
#endif /* defined(INCLUDE_LONG_LONG) || INCLUDE_LONG_LONG_ERROR */
				}
			}
			...
			for (p = (char *) u_spec ; *p ; p++) {
				...
				if (p-u_spec < 8) { /* unsigned conversion */
					...
					if (lval >= 2) {
						...
						p = (char *) ll_err;
#endif /* defined(INCLUDE_LONG_LONG) */
					} else {
#endif /* defined(INCLUDE_LONG_LONG) || INCLUDE_LONG_LONG_ERROR */
...
#endif /* defined(INCLUDE_FLOAT) || INCLUDE_FLOAT_ERROR */
#if defined(INCLUDE_FLOAT)
					if (preci < 0) {
						preci = 6;
					}
					cnt += __dtostr(op, 
		...
		charout:
		if (++cnt < max_size) {
			__sputc(*fmt, op);	/* normal char out */
		}
