...
UGL_GC_ID uglGcCreate(UGL_DEVICE_ID devId)
{
  UGL_GC_ID gc;
  UGL_POS width, height;
...
UGL_STATUS uglGcCopy(UGL_GC_ID src, UGL_GC_ID dest)
{
  UGL_DEVICE_ID devId;
...
UGL_STATUS uglGcSet(UGL_DEVICE_ID devId, UGL_GC_ID gc)
{
  /* Lock device */
  if (uglOsLock(devId->lockId) != UGL_STATUS_OK)
    return UGL_STATUS_ERROR;
